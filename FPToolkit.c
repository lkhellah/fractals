...
/*Version 105:
  ...
  void G_hsl(double h, double s, double l)
      for hue/saturation/lightness as an alternative to G_rgb
 
  int G_click_and_save(double x[], double y[])
...
In an HSL color model, HSL stands for:

H (hue) : Hue is the color's position on the color wheel, measured in 
          degrees.

S (saturation) : Saturation represents a colors purtiy.  The higher the 
                 saturation the purer the color.  Scale is 0% to 100%

L (lightness) : Lightness represents the brightness of the color. As lightness
                increases the color appears brighter.  Scale is 0% to 100%

This program converts HSL space to RGB space using the following:
...
{
  // NOTE:  c is chroma
  //        Hue is converted to a segment of the color wheel, 0 through 6.
  //        This segment determines which face of the RGB cube contains the color.
  
  if (s < 0) {
    printf("Error : Saturation of %lf out of range 0 <= s <= 100, value made 0\n", s) ;
    s = 0 ; ;
  }

  if (s > 100) {
    printf("Error : Saturation of %lf out of range 0 <= s <= 100, value made 100\n", s) ;
    s = 100 ; ;
  }

  if (l < 0) {
    printf("Error : Lightness of %lf out of range 0 <= l <= 100, value made 0\n", l) ;
    l = 0 ; ;
  }

  if (l > 100) {
    printf("Error : Lightness of %lf out of range 0 <= l <= 100, value made 100\n", l) ;
    l = 100 ; ;
  }  
  ...
  s = s/100.0 ;  l = l/100.0 ; // convert from percentage to decimal
...
double G_click_and_save_button_lower_left_x = 0 ;
double G_click_and_save_button_lower_left_y = 0 ;

double G_click_and_save_button_width  = 60 ;
double G_click_and_save_button_height = 40 ;

double G_click_and_save_button_red = 0.33 ;
double G_click_and_save_button_grn = 0.79 ;
double G_click_and_save_button_blu = 0.24 ;

double G_click_and_save_red = 0.92 ;
double G_click_and_save_grn = 0.53 ;
double G_click_and_save_blu = 0.72 ;

double G_click_and_save_dot_radius = 3 ;
int    G_click_and_save_draw_dots = 1 ; // 1 for yes, else don't draw
int    G_click_and_save_draw_lines = 1 ; // 1 for yes, else don't draw
...
int G_click_and_save (double x[], double y[])
{
  ...
  G_rgb(G_click_and_save_button_red,
	G_click_and_save_button_grn,
	G_click_and_save_button_blu) ;
  G_fill_rectangle(
      G_click_and_save_button_lower_left_x,
      G_click_and_save_button_lower_left_y,
      G_click_and_save_button_width,
      G_click_and_save_button_height) ;

  
  G_rgb(G_click_and_save_red,
	G_click_and_save_grn,
	G_click_and_save_blu) ;
  G_wait_click(P);
  ...
  while (1) {
    ...
    if (
         P[0] >= G_click_and_save_button_lower_left_x
	 &&
         P[0] <  G_click_and_save_button_lower_left_x +
                 G_click_and_save_button_width
	 &&
         P[1] >= G_click_and_save_button_lower_left_y
	 &&
         P[1] <  G_click_and_save_button_lower_left_y +
                 G_click_and_save_button_height
    ...
    if (G_click_and_save_draw_dots == 1) {    
      G_fill_circle(x[n],y[n],G_click_and_save_dot_radius) ;
    }
    
    if (G_click_and_save_draw_lines == 1) {
